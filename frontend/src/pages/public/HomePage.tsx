      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div>
              <span className={`font-bold text-lg leading-none block ${scrolled?"text-gray-900":"text-white"}`}>MYBRIDGE</span>
              <span className={`text-[9px] leading-none block ${scrolled?"text-gray-400":"text-white/60"}`}>The link you need</span>
            </div>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navLinks.map(link=>(
              <a key={link.key} href={link.href}
                ...
                className={`px-3 py-2 text-sm font-medium rounded-lg transition-colors ${scrolled?"text-gray-600 hover:text-[#FF6B35] hover:bg-orange-50":"text-white/90 hover:text-white hover:bg-white/10"}`}>
                {link.label}
              </a>
            ))}
          </div>
          ...
          <div className={`flex items-center gap-2 ${isRTL?'flex-row-reverse':''}`}>
            <div className="relative">
              <button onClick={()=>setLangOpen(!langOpen)}
                className={`flex items-center gap-1.5 text-xs font-medium px-3 py-2 rounded-xl border transition-all ${scrolled?"border-gray-200 text-gray-600 hover:border-[#FF6B35] hover:text-[#FF6B35]":"border-white/30 text-white hover:bg-white/10"}`}>
                ...
                <div className="absolute top-full mt-1 right-0 bg-white border border-gray-100 rounded-xl shadow-lg overflow-hidden z-50 min-w-[140px]">
                  {LANGUAGES.map(l=>(
                    <button key={l.code} onClick={()=>{ onLangChange(l.code); setLangOpen(false); }}
                      className={`w-full text-left px-4 py-2.5 text-sm transition-colors hover:bg-orange-50 hover:text-[#FF6B35] ${lang===l.code?"text-[#FF6B35] font-semibold bg-orange-50":"text-gray-700"}`}>
                      {l.label}
                    </button>
                  ))}
                </div>
            ...
            <div className="hidden lg:flex items-center gap-1.5">
              <button onClick={onSupplier}
                className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-xl border transition-all ${scrolled?"border-[#FF6B35] text-[#FF6B35] hover:bg-orange-50":"border-white/40 text-white hover:bg-white/10"}`}>
...
// ═══════════════════════════════════════════════════════
// HERO — يستخدم card_image في البطاقة و hero_image في الخلفية
// ═══════════════════════════════════════════════════════
const HeroSection = ({ t, isRTL }: { t:(k:string)=>string; isRTL:boolean }) => {
  ...
  if (isLoading) {
    return (
      <section className="relative min-h-[680px] flex items-center justify-center bg-[#1A1A2E]">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className={isRTL?"text-right":""}>
            <span className="inline-block bg-[#FF6B35]/20 text-orange-300 text-xs font-semibold px-3 py-1.5 rounded-full mb-4 border border-[#FF6B35]/30">
            ...
            <h1 className="text-5xl md:text-6xl font-bold text-white leading-tight mb-5">
              {t("hero.title")}<br/>
              <span className="text-[#FF6B35]">{t("hero.titleSpan")}</span>
            </h1>
  ...
  <section className="py-20 bg-white" dir={isRTL?"rtl":"ltr"}>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className={`mb-10 ${isRTL?"text-right":""}`}>
        <p className="text-[#FF6B35] text-sm font-semibold uppercase tracking-wide mb-1">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-[#FF6B35] rounded-2xl p-6 flex flex-col justify-between hover:bg-[#e07a38] transition-colors cursor-pointer">
          <p className="text-white font-bold text-sm leading-snug">Crafting Your Perfect Travel Experience</p>
          <a href="/destinations" className="mt-4 bg-white text-[#FF6B35] text-xs font-bold px-4 py-2.5 rounded-xl flex items-center gap-1.5 hover:bg-orange-50 transition-colors w-fit">
...
const MalaysiaSection = ({ t, isRTL }: { t:(k:string)=>string; isRTL:boolean }) => (
  <section className="py-20 bg-[#1A1A2E]" dir={isRTL?"rtl":"ltr"}>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className={isRTL?"text-right":""}>
          <p className="text-[#FF6B35] text-sm font-semibold uppercase tracking-wide mb-4">
            <FontAwesomeIcon icon={faMapMarkedAlt} className={isRTL?"ml-2":"mr-2"}/>
            {t("malaysia.label")}
          </p>
          <h2 className="text-4xl font-bold text-white mb-4 leading-tight">
            {t("malaysia.title")}<br/>
            <span className="text-[#FF6B35]">{t("malaysia.titleSpan")}</span>
          </h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={s.num} className="mb-stat bg-white/5 border border-white/10 rounded-2xl p-5 text-center hover:bg-white/10 transition-colors">
                <div className="text-2xl font-bold text-[#FF6B35] mb-1">{s.num}</div>
  ...
  <section className="py-20 bg-gray-50" dir={isRTL?"rtl":"ltr"}>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className={`flex items-end justify-between mb-10 ${isRTL?"flex-row-reverse":""}`}>
        <div className={isRTL?"text-right":""}>
          <p className="text-[#FF6B35] text-sm font-semibold uppercase tracking-wide mb-1">{t("news.label")}</p>
          <h2 className="text-3xl font-bold text-gray-900">{t("news.title")}</h2>
        </div>
...
const NewsletterSection = ({ t, isRTL }: { t:(k:string)=>string; isRTL:boolean }) => {
  ...
  return (
    <section className="py-20 bg-white" dir={isRTL?"rtl":"ltr"}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-[#1A1A2E] rounded-3xl p-10 md:p-16 grid grid-cols-1 md:grid-cols-2 gap-10 items-center relative overflow-hidden">
